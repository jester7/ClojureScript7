(ns clojurescript7.components.cells
  (:require
   [clojurescript7.components.cells.utility :as u :refer
  ...
  [:div#cells.applet
   ...
   [:div#cells-toolbar.row.wrapper
    ...
    [:input#formula-input {:value @current-formula
                           :on-change #()}]]
   [:div#spreadsheet.wrapper (u/keyboard-navigation)
             ...
             [:div.row.wrapper {:key (str "row" row)}
              ...
              (doall (for [col (range 1 max-cols)]
                       (if (= row 0)
                         ...
                         [:input {:read-only true :key (u/cell-ref row col) :data-row row :data-col col :id (u/cell-ref row col)
                                  :on-click
                                  ...
                                  (fn [e]
                                    ...
                                    (let [val (-> e .-target .-value)
                                          ...
                                          cell-ref (u/cell-ref row col)]
                                      (when (not= val "") ; don't update the map if value is empty string
                                        (if (has-formula? cell-ref)
                                          (do ; TODO problem is here
                                            (let [c-map {:formula (if (u/is-formula? val) val (:formula (@cells-map cell-ref)))  ;(or (:formula (@cells-map cell-ref)) val)
                                                         :format ""
                                                         :value (if (u/is-formula? val) (u/parse-formula val) val)}]
