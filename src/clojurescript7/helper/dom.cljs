(ns clojurescript7.helper.dom
  (:require
   [clojure.string :as s]
   [clojurescript7.helper.collections :as c]))
...
;; (defn animate-then [el-id post-animation-fn]
;;   ;;   (.getElementById js/document el-id)
;;   (let [element        (js/document.getElementById el-id)
;;         style       (.-style element)
;;         transform     (if (= (.-transform style) "") "rotate(0deg)" (.-transform style))
;;         new-transform (if (= "rotate(0deg)" transform) "rotate(360deg)" "rotate(0deg)")]
;;     (js/console.log transform new-transform)
;;     (set! (.-transform style) new-transform))
;;   (post-animation-fn))
